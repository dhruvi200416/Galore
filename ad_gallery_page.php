<?php
include 'ad_gallery_page_handler.php';
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
            <h1 class="mb-0">Gallery Page Management</h1>
            <!-- <button class="btn-add-gallery" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Gallery Page
            </button> -->
        </div>

        <!-- FORM -->
        <div class="add-gallery-form-container" id="addGalleryForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Gallery Page</h3>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Gallery Page Full Details</h5>
    ...
    <script>
        // Use PHP to pass database galleries to JavaScript
        let galleryData = <?php echo json_encode($galleryData); ?>;

        // If no galleries in database, use empty array
        ...
        // Open form button
        $("#openFormBtn").click(() => {
            formTitleText.text("Add Gallery Page");
            editId = null;
            $("#edit_id").val("");
            $("#galleryForm")[0].reset();
        ...
        function editGallery(id) {
            ...
            formTitleText.text("Edit Gallery Page");
            $("#edit_id").val(id);
        ...
        // Remove the media query by using Bootstrap classes for responsive form layout
        function applyResponsiveForm() {
            const windowWidth = $(window).width();
            
            // Make form 2 columns on desktop, 1 column on mobile
            if (windowWidth >= 768) {
                $('#galleryForm').css('grid-template-columns', '1fr 1fr');
                $('.form-buttons').css('grid-column', 'span 2');
            } else {
                $('#galleryForm').css('grid-template-columns', '1fr');
                $('.form-buttons').css('grid-column', '1');
            }
        }
