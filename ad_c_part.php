...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0"><i class="fas fa-users me-2"></i>Participation Management</h1>
            <!-- <button class="btn-add-part" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Participation Page
            </button> -->
        </div>

        <!-- ADD / EDIT PARTICIPATION PAGE FORM -->
        <div class="add-part-form-container" id="addPartForm" style="display:none;">
        ...
        <div class="data-table-container">
            <div class="table-header" id="tableHeader">
                <h2 class="mb-0">Participation Pages List</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold"><i class="fas fa-info-circle me-2"></i>Participation Page Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="text-center mb-4">
                        <i class="fas fa-users" style="font-size: 4rem; color: var(--galore-red); opacity: 0.5;"></i>
                    </div>
                    <div class="row g-3">
                        <div class="col-12">
    ...
    <script>
        // Use PHP to pass database participation pages to JavaScript
        let partData = <?php echo json_encode($partData); ?>;

        // If no participation pages in database, use empty array
        ...
        $("#openFormBtn").click(() => {
            ...
            addPartForm.slideDown(300);
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addPartForm.slideUp(300);
            $("#partForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#partTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = partData.filter(p =>
                p.hero_title.toLowerCase().includes(searchValue) ||
                p.hero_subtitle.toLowerCase().includes(searchValue)
            );
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(p => {
                    const statusClass = p.status === 'Active' ? 'btn-status-active' : 'btn-status-inactive';
                    
                    tbody.append(`
                    <tr>
                        <td><span">${p.id}</span></td>
                        <td>${p.hero_title}</td>
                        <td>${p.hero_subtitle}</td>
                        <td>
                        ...
                        <td>
                            <button class="action-btn btn-view" onclick="viewPart(${p.id})"> View</button>
                            <button class="action-btn btn-edit" onclick="editPart(${p.id})"> Edit</button>
                            <button class="action-btn btn-delete" onclick="deletePart(${p.id})"> Delete</button>
                        </td>
                    </tr>`);
                });
            }
        ...
        // View participation page details
        window.viewPart = function(id) {
            let p = partData.find(x => x.id == id);
            $("#v_id").text(p.id);
            $("#v_hero_title").text(p.hero_title);
            $("#v_hero_subtitle").text(p.hero_subtitle);
            $("#v_status").text(p.status);
            new bootstrap.Modal(document.getElementById("viewModal")).show();
        };

        // Delete participation page
        ...
        window.editPart = function(id) {
            editId = id;
            const p = partData.find(x => x.id == id);

            formTitleText.text("Edit Participation Page");
            $("#edit_id").val(id);

            // Fill form fields
            $('input[name="hero_title"]').val(p.hero_title);
            $('input[name="hero_subtitle"]').val(p.hero_subtitle);
        ...
        $(document).ready(function() {
            ...
            // Participation form submission with validation
        ...
        $(document).keydown(function(e) {
            if (e.key === "Escape" && addPartForm.is(":visible")) {
                addPartForm.slideUp(300);
                $("#partForm")[0].reset();
                $("#edit_id").val("");
                $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
