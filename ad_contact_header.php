<?php
include 'ad_contact_header_handler.php';
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0">Contact Header Management</h1>
            <!-- <button class="btn-add-user" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Contact Header
            </button> -->
        </div>

        <!-- FORM -->
        <div class="add-user-form-container" id="addContactHeaderForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Contact Header</h3>

            <form id="contactHeaderForm" method="POST" action="">
                <input type="hidden" name="edit_id" id="edit_id" value="">

                <div class="galore-input-group">
                    <label class="galore-label">Hero Title <span class="text-danger">*</span></label>
                    <input type="text" name="hero_title" class="galore-input" data-validation="required min alphabetic" data-min="3" data-max="100">
                ...
                <div class="galore-input-group">
                    ...
                    <input type="text" name="hero_subtitle" class="galore-input" data-validation="required min" data-min="3" data-max="200">
                    <span id="hero_subtitle_error" class="error-message"></span>
                </div>

                <div class="galore-input-group">
                    <label class="galore-label">Form Title <span class="text-danger">*</span></label>
                ...
                <div class="galore-input-group">
                    <label class="galore-label">Form Subtitle <span class="text-danger">*</span></label>
                    <input type="text" name="form_subtitle" class="galore-input" data-validation="required min" data-min="3" data-max="200">
                    <span id="form_subtitle_error" class="error-message"></span>
                </div>

                <div class="galore-input-group">
                    <label class="galore-label">Footer Note <span class="text-danger">*</span></label>
                    <input type="text" name="footer_note" class="galore-input" data-validation="required min" data-min="3" data-max="200">
        ...
        <div class="data-table-container">
            ...
            <div class="table-header" id="tableHeader">
                <h2 class="mb-0">Contact Header Data</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Contact Header Full Details</h5>
                ...
                <div class="modal-body text-center">
                    ...
                    <p><b>Form Subtitle:</b> <span id="v_form_subtitle"></span></p>
    ...
    <script>
        ...
        let contactHeaderData = <?php echo json_encode($contactHeaderData); ?>;

        // If no headers in database, use empty array
        if (!contactHeaderData || contactHeaderData.length === 0) {
            contactHeaderData = [];
        }
        ...
        const addContactHeaderForm = $("#addContactHeaderForm");
        const formTitleText = $("#formTitleText");

        // Open form button
        $("#openFormBtn").click(() => {
            formTitleText.text("Add Contact Header");
            editId = null;
            $("#edit_id").val("");
            ...
            $(".error-message").text("");
            addContactHeaderForm.slideDown();
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addContactHeaderForm.slideUp();
            $("#contactHeaderForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#contactTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = contactHeaderData.filter(h =>
                h.hero_title.toLowerCase().includes(searchValue) ||
                h.hero_subtitle.toLowerCase().includes(searchValue) ||
                h.form_title.toLowerCase().includes(searchValue)
            );
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(h => {
                    ...
                    <tr>
                        <td>${h.id}</td>
                        <td>${h.hero_title}</td>
                        <td>${h.hero_subtitle}</td>
                        <td>${h.form_title}</td>
                        <td>
                            <button class="action-btn btn-view" onclick="viewContactHeader(${h.id})">View</button>
                            <button class="action-btn btn-edit" onclick="editContactHeader(${h.id})">Edit</button>
                            <button class="action-btn btn-delete" onclick="deleteContactHeader(${h.id})">Delete</button>
                        </td>
                    </tr>`);
                });
            }
        ...
        // View contact header details
        function viewContactHeader(id) {
            let h = contactHeaderData.find(x => x.id == id);
            $("#v_id").text(h.id);
            $("#v_hero_title").text(h.hero_title);
            $("#v_hero_subtitle").text(h.hero_subtitle);
            $("#v_form_title").text(h.form_title);
            $("#v_form_subtitle").text(h.form_subtitle);
        ...
        function deleteContactHeader(id) {
            if (confirm("Are you sure you want to delete this contact header?")) {
        ...
        function editContactHeader(id) {
            editId = id;
            const h = contactHeaderData.find(x => x.id == id);
            formTitleText.text("Edit Contact Header");
            $("#edit_id").val(id);
            ...
            $('input[name="form_subtitle"]').val(h.form_subtitle);
        ...
        $(document).ready(function() {
            ...
            // Contact header form submission with validation
            $("#contactHeaderForm").on("submit", function(e) {
                let isValid = true;
