...
// Create co_dash table
$dash_table = "CREATE TABLE IF NOT EXISTS co_dash (
...
if (mysqli_query($con, $dash_table)) {
    // echo "Table 'co_dash' created successfully";
} else {
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        // Update existing dashboard - status not updated from form
        $edit_id = mysqli_real_escape_string($con, $_POST['edit_id']);

        $update_query = "UPDATE co_dash SET 
        ...
        // Insert new dashboard - status defaults to 'Active' from table definition
        $insert_query = "INSERT INTO co_dash (hero_title, hero_subtitle) 
...
if (isset($_GET['toggle_id'])) {
    ...
    $toggle_query = "UPDATE co_dash SET status = IF(status = 'Active', 'Inactive', 'Active') WHERE id = '$toggle_id'";
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM co_dash WHERE id = '$delete_id'";
...
$dashData = [];
$select_query = "SELECT * FROM co_dash ORDER BY id ASC";
