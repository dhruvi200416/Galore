...
// Create co_sch table
$sch_table = "CREATE TABLE IF NOT EXISTS co_sch (
...
if (mysqli_query($con, $sch_table)) {
    // echo "Table 'co_sch' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Handle form submission for school page
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        // Update existing school - status not updated from form
        $edit_id = mysqli_real_escape_string($con, $_POST['edit_id']);

        $update_query = "UPDATE co_sch SET 
        ...
        // Insert new school - status defaults to 'Active' from table definition
        $insert_query = "INSERT INTO co_sch (hero_title, hero_subtitle) 
...
if (isset($_GET['toggle_id'])) {
    ...
    $toggle_query = "UPDATE co_sch SET status = IF(status = 'Active', 'Inactive', 'Active') WHERE id = '$toggle_id'";
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM co_sch WHERE id = '$delete_id'";
...
$schoolData = [];
$select_query = "SELECT * FROM co_sch ORDER BY id ASC";
