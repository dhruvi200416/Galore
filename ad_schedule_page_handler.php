...
// Create schedule_page table
$schedule_table = "CREATE TABLE IF NOT EXISTS schedule_page (
...
if (mysqli_query($con, $schedule_table)) {
    // echo "Table 'schedule_page' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Handle form submission for schedule page
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE schedule_page SET 
        ...
        if (mysqli_query($con, $update_query)) {
            ...
            echo "<script>alert('Error updating schedule: " . mysqli_error($con) . "');</script>";
        }
    } else {
        // Insert new schedule
        $insert_query = "INSERT INTO schedule_page (hero_title, hero_subtitle) 
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM schedule_page WHERE id = '$delete_id'";
...
// Fetch all schedules from database for display
$scheduleData = [];
$select_query = "SELECT * FROM schedule_page ORDER BY id ASC";
