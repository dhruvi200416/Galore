...
include 'ad_home_events_handler.php';
?>
...
<head>
    ...
    <style>
        ...
        /* Image Preview Styling - Rectangle with rounded corners */
        ...
        /* Table Image Styling - Rectangle with rounded corners */
        ...
        /* Modal Image Styling - Rectangle with rounded corners */
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar">
            <h1 class="mb-0">Home Events Management</h1>
        ...
        <div class="add-user-form-container" id="addUserForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add New Event</h3>

            <form id="userForm" method="POST" enctype="multipart/form-data" action="ad_home_events_handler.php">
                ...
                <div class="galore-input-group">
                    ...
                    <input type="text" name="description" class="galore-input" data-validation="required min" data-min="3" data-max="500">
                    <span id="description_error" class="error-message"></span>
                </div>

                <div class="galore-input-group">
                    <label class="galore-label">Button Link <span class="text-danger">*</span></label>
                    <input type="text" name="btn_link" class="galore-input" data-validation="required min" data-min="3" data-max="100">
                    <span id="btn_link_error" class="error-message"></span>
                </div>

                <div class="galore-input-group">
                    <label class="galore-label">Button Text <span class="text-danger">*</span></label>
                    <input type="text" name="btn_text" class="galore-input" data-validation="required min alphabetic" data-min="3" data-max="100">
                    <span id="btn_text_error" class="error-message"></span>
                </div>
        ...
        <div class="data-table-container">
            <div class="table-header">
                <h2 class="mb-0">Home Events Data</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Events Full Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>

                <div class="modal-body text-center">
                    <div id="v_image_container" class="modal-image-container" style="display:none;">
                        <img id="v_image" class="modal-image" src="#" alt="Event">
                    </div>
                    <p><b>ID:</b> <span id="v_id"></span></p>
                    <p><b>Heading:</b> <span id="v_heading"></span></p>
                    <p><b>Description:</b> <span id="v_description"></span></p>
                    <p><b>Button Link:</b> <span id="v_link"></span></p>
                    <p><b>Button Text:</b> <span id="v_text"></span></p>
    ...
    <script>
        ...
        $("#openFormBtn").click(() => {
            formTitleText.text("Add New Event");
            editId = null;
            isEditing = false;
            $("#edit_id").val("");
            $("#userForm")[0].reset();
        ...
        function renderTable() {
            ...
            const filteredData = userData.filter(u =>
                String(u.heading).toLowerCase().includes(searchValue) ||
                String(u.description).toLowerCase().includes(searchValue) ||
                String(u.button_link).toLowerCase().includes(searchValue)
            );
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(u => {
                    const imageDisplay = u.image && u.image.trim() !== "" ? 
                        `<div class="table-image-container">
                            <img src="${u.image}" class="table-image" alt="${u.heading}">
                        </div>` : 
                        "<div class='table-image-container' style='background:var(--galore-gray);color:white;font-size:20px;display:flex;align-items:center;justify-content:center;'><i class='bi bi-image'></i></div>";
                    ...
                    tbody.append(`
                        <tr>
                            ...
                            <td>${u.heading}</td>
                            <td>
                                <button class="btn-status ${u.status === 'Active' ? 'btn-status-active' : 'btn-status-inactive'}" 
        ...
        function toggleStatus(id) {
            window.location.href = "ad_home_events_handler.php?toggle_id=" + id;
        }
        ...
        function editUser(id) {
            ...
            $('input[name="heading"]').val(user.heading);
            $('input[name="description"]').val(user.description);
            $('input[name="btn_link"]').val(user.button_link);
            $('input[name="btn_text"]').val(user.button_text);
            ...
            if (user.image) {
                ...
                imagePreview.src = currentImage;
                imagePreviewContainer.style.display = "flex";
                $("#file-name").text("Current image: " + user.image.split('/').pop());
            }
        ...
        function deleteUser(id) {
            if (confirm("Are you sure you want to delete this event?")) {
                window.location.href = "ad_home_events_handler.php?delete_id=" + id;
            }
        }
        ...
        function viewUser(id) {
            ...
            if (user) {
                ...
                if (user.image) {
                    vImage.attr("src", user.image);
                    vImageContainer.show();
                } else {
                    vImageContainer.hide();
                }
                
                $("#v_heading").text(user.heading);
                $("#v_description").text(user.description);
                $("#v_link").text(user.button_link);
                $("#v_text").text(user.button_text);
        ...
        $(document).keydown(function(e) {
            if (e.key === "Escape" && addUserForm.is(":visible")) {
                addUserForm.slideUp();
                $("#userForm")[0].reset();
                $("#file-name").text("Choose file...");
                imagePreviewContainer.style.display = "none";
