<?php
include 'ad_result_header_handler.php';
?>
...
<head>
    ...
    <style>
        ...
        /* ACTION BUTTONS - Simple styling as requested */
        .action-btn {
            padding: 8px 15px;
            border: none;
            border-radius: 8px;
        ...
        /* Only keeping the sidebar media query */
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0">Result Header Management</h1>
            <!-- <button class="btn-add-user" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Result Header
            </button> -->
        </div>

        <!-- FORM -->
        <div class="add-user-form-container" id="addResultHeaderForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Result Header</h3>

            <form id="resultHeaderForm" method="POST" action="">
                ...
                <div class="galore-input-group">
                    <label class="galore-label">Title</label>
        ...
        <div class="data-table-container">
            ...
            <div class="table-responsive">
                <table class="data-table">
                    ...
                    <tbody id="resultHeaderTableBody"></tbody>
                </table>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Result Header Full Details</h5>
    ...
    <script>
        ...
        let resultHeaderData = <?php echo json_encode($resultHeaderData); ?>;

        // If no headers in database, use empty array
        if (!resultHeaderData || resultHeaderData.length === 0) {
            resultHeaderData = [];
        }
        ...
        const addResultHeaderForm = $("#addResultHeaderForm");
        const formTitleText = $("#formTitleText");

        // Open form button
        $("#openFormBtn").click(() => {
            formTitleText.text("Add Result Header");
            editId = null;
            $("#edit_id").val("");
            $("#resultHeaderForm")[0].reset();
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
            $(".error-message").text("");
            addResultHeaderForm.slideDown();
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addResultHeaderForm.slideUp();
            $("#resultHeaderForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#resultHeaderTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = resultHeaderData.filter(r =>
                r.hero_title.toLowerCase().includes(searchValue) ||
                r.hero_subtitle.toLowerCase().includes(searchValue) ||
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(r => {
                    ...
                    <tr>
                        <td>${r.id}</td>
                        <td>${r.hero_title}</td>
                        <td>${r.hero_subtitle}</td>
                        <td>${r.title}</td>
                        <td>
                            <div class="d-flex flex-column flex-sm-row gap-2 gap-sm-2">
                                <button class="action-btn btn-view" onclick="viewResultHeader(${r.id})">View</button>
                                <button class="action-btn btn-edit" onclick="editResultHeader(${r.id})">Edit</button>
                                <button class="action-btn btn-delete" onclick="deleteResultHeader(${r.id})">Delete</button>
                            </div>
                        </td>
                    </tr>`);
                });
        ...
        // View result header details
        function viewResultHeader(id) {
            let r = resultHeaderData.find(x => x.id == id);
            $("#v_id").text(r.id);
            $("#v_hero_title").text(r.hero_title);
            $("#v_hero_subtitle").text(r.hero_subtitle);
            $("#v_title").text(r.title);
            new bootstrap.Modal(document.getElementById("viewModal")).show();
        }

        // Delete result header
        function deleteResultHeader(id) {
            if (confirm("Are you sure you want to delete this result header?")) {
                window.location.href = "?delete_id=" + id;
            }
        }

        // Edit result header
        function editResultHeader(id) {
            editId = id;
            const r = resultHeaderData.find(x => x.id == id);
            formTitleText.text("Edit Result Header");
            $("#edit_id").val(id);
            
            // Set form values
            $('input[name="hero_title"]').val(r.hero_title);
            $('input[name="hero_subtitle"]').val(r.hero_subtitle);
        ...
        $(document).ready(function() {
            ...
            // Result header form submission with validation
            $("#resultHeaderForm").on("submit", function(e) {
                let isValid = true;
