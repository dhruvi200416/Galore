...
// Create cultural_header table
$cultural_header_table = "CREATE TABLE IF NOT EXISTS cultural_header (
...
if (mysqli_query($con, $cultural_header_table)) {
    // echo "Table 'cultural_header' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Create cultural_event table
$cultural_event_table = "CREATE TABLE IF NOT EXISTS cultural_event (
    id INT(11) AUTO_INCREMENT PRIMARY KEY,
...
if (mysqli_query($con, $cultural_event_table)) {
    // echo "Table 'cultural_event' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Handle status toggle request for cultural events
if (isset($_GET['toggle_event_id'])) {
    $toggle_id = mysqli_real_escape_string($con, $_GET['toggle_event_id']);

    $toggle_query = "UPDATE cultural_event SET status = IF(status = 'Active', 'Inactive', 'Active') WHERE id = '$toggle_id'";
...
// Handle form submission for cultural header
if (isset($_POST['submit_header'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE cultural_header SET 
        ...
        // Insert new header
        $insert_query = "INSERT INTO cultural_header (hero_title, hero_subtitle, section_title, section_subtitle, note_text) 
...
// Handle form submission for cultural event - REMOVED STATUS FROM FORM
if (isset($_POST['submit_event'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE cultural_event SET 
        ...
        $insert_query = "INSERT INTO cultural_event (event_name, description, status) 
...
// Handle delete request for cultural header
if (isset($_GET['delete_header_id'])) {
    $delete_id = mysqli_real_escape_string($con, $_GET['delete_header_id']);

    $delete_query = "DELETE FROM cultural_header WHERE id = '$delete_id'";
...
// Handle delete request for cultural event
if (isset($_GET['delete_event_id'])) {
    $delete_id = mysqli_real_escape_string($con, $_GET['delete_event_id']);

    $delete_query = "DELETE FROM cultural_event WHERE id = '$delete_id'";
    if (mysqli_query($con, $delete_query)) {
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
...
$sportData = [];
$select_query = "SELECT * FROM cultural_header ORDER BY id ASC";
...
$eventData = [];
$select_query = "SELECT * FROM cultural_event ORDER BY id ASC";
$result = mysqli_query($con, $select_query);
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        $eventData[] = $row;
