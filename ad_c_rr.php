...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0"><i class="fas fa-trophy me-2"></i>Results/Rankings Management</h1>
            <button class="btn-add-rr" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Result Page
            </button>
        </div>

        <!-- ADD / EDIT RESULT PAGE FORM -->
        <div class="add-rr-form-container" id="addRRForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Result Page</h3>

            <form id="rrForm" method="POST" action="">
        ...
        <div class="data-table-container">
            <div class="table-header" id="tableHeader">
                <h2 class="mb-0">Result Pages List</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold"><i class="fas fa-info-circle me-2"></i>Result Page Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="text-center mb-4">
                        <i class="fas fa-trophy" style="font-size: 4rem; color: var(--galore-red); opacity: 0.5;"></i>
                    </div>
                    <div class="row g-3">
                        <div class="col-12">
    ...
    <script>
        // Use PHP to pass database result pages to JavaScript
        let rrData = <?php echo json_encode($rrData); ?>;

        // If no result pages in database, use empty array
        ...
        $("#openFormBtn").click(() => {
            ...
            addRRForm.slideDown(300);
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addRRForm.slideUp(300);
            $("#rrForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#rrTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = rrData.filter(r =>
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(r => {
                    ...
                    <tr>
                        ...
                        <td>
                            <button class="btn-status ${statusClass}" onclick="toggleStatus(${r.id})">
                                ${r.status}
                            </button>
                        </td>
                        <td>
                            <button class="action-btn btn-view" onclick="viewRR(${r.id})"> View</button>
                            <button class="action-btn btn-edit" onclick="editRR(${r.id})"> Edit</button>
                            <button class="action-btn btn-delete" onclick="deleteRR(${r.id})"> Delete</button>
                        </td>
                    </tr>`);
                });
            }
        ...
        // View result page details
        window.viewRR = function(id) {
            let r = rrData.find(x => x.id == id);
            $("#v_id").text(r.id);
            $("#v_hero_title").text(r.hero_title);
            $("#v_hero_subtitle").text(r.hero_subtitle);
            $("#v_status").text(r.status);
            new bootstrap.Modal(document.getElementById("viewModal")).show();
        };

        // Delete result page
        window.deleteRR = function(id) {
            if (confirm("Are you sure you want to delete this result page?")) {
                window.location.href = "?delete_id=" + id;
            }
        };

        // Edit result page
        window.editRR = function(id) {
            editId = id;
            const r = rrData.find(x => x.id == id);
        ...
        $(document).keydown(function(e) {
            if (e.key === "Escape" && addRRForm.is(":visible")) {
                addRRForm.slideUp(300);
                $("#rrForm")[0].reset();
                $("#edit_id").val("");
                $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
