...
// Create c_part table
$part_table = "CREATE TABLE IF NOT EXISTS c_part (
...
if (mysqli_query($con, $part_table)) {
    // echo "Table 'c_part' created successfully";
} else {
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE c_part SET 
        ...
        if (mysqli_query($con, $update_query)) {
            ...
            echo "<script>alert('Error updating participation: " . mysqli_error($con) . "');</script>";
        }
    } else {
        // Insert new participation
        $insert_query = "INSERT INTO c_part (hero_title, hero_subtitle) 
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM c_part WHERE id = '$delete_id'";
    if (mysqli_query($con, $delete_query)) {
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
        echo "<script>alert('Error deleting participation: " . mysqli_error($con) . "');</script>";
    }
}

// Fetch all participations from database for display
$partData = [];
$select_query = "SELECT * FROM c_part ORDER BY id ASC";
