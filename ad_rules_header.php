<?php
include 'ad_rules_header_handler.php';
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0">Rules Header Management</h1>
            <!-- <button class="btn-add-user" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Rules Header
            </button> -->
        </div>

        <!-- FORM -->
        <div class="add-user-form-container" id="addRulesHeaderForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Rules Header</h3>

            <form id="rulesHeaderForm" method="POST" action="">
        ...
        <div class="data-table-container">
            ...
            <div class="table-responsive">
                <table class="data-table">
                    ...
                    <tbody id="rulesHeaderTableBody"></tbody>
                </table>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Rules Header Full Details</h5>
    ...
    <script>
        ...
        let rulesHeaderData = <?php echo json_encode($rulesHeaderData); ?>;

        // If no headers in database, use empty array
        if (!rulesHeaderData || rulesHeaderData.length === 0) {
            rulesHeaderData = [];
        }
        ...
        const addRulesHeaderForm = $("#addRulesHeaderForm");
        const formTitleText = $("#formTitleText");

        // Open form button
        $("#openFormBtn").click(() => {
            formTitleText.text("Add Rules Header");
            editId = null;
            $("#edit_id").val("");
            $("#rulesHeaderForm")[0].reset();
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
            $(".error-message").text("");
            addRulesHeaderForm.slideDown();
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addRulesHeaderForm.slideUp();
            $("#rulesHeaderForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#rulesHeaderTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = rulesHeaderData.filter(r =>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex flex-column flex-sm-row gap-2 gap-sm-2">
                                <button class="action-btn btn-view" onclick="viewRulesHeader(${r.id})">View</button>
                                <button class="action-btn btn-edit" onclick="editRulesHeader(${r.id})">Edit</button>
                                <button class="action-btn btn-delete" onclick="deleteRulesHeader(${r.id})">Delete</button>
                            </div>
                        </td>
                    </tr>`);
                });
        ...
        // View rules header details
        function viewRulesHeader(id) {
            let r = rulesHeaderData.find(x => x.id == id);
            $("#v_id").text(r.id);
        ...
        // Delete rules header
        function deleteRulesHeader(id) {
            if (confirm("Are you sure you want to delete this rules header?")) {
                window.location.href = "?delete_id=" + id;
            }
        }

        // Edit rules header
        function editRulesHeader(id) {
            editId = id;
            const r = rulesHeaderData.find(x => x.id == id);
            formTitleText.text("Edit Rules Header");
            $("#edit_id").val(id);
        ...
        $(document).ready(function() {
            ...
            // Rules header form submission with validation
            $("#rulesHeaderForm").on("submit", function(e) {
                let isValid = true;
