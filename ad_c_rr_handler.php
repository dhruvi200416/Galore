...
// Create co_rr table (for Results/Rankings)
$rr_table = "CREATE TABLE IF NOT EXISTS co_rr (
...
if (mysqli_query($con, $rr_table)) {
    // echo "Table 'co_rr' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Handle form submission for result page
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        // Update existing result page - status not updated from form
        $edit_id = mysqli_real_escape_string($con, $_POST['edit_id']);

        $update_query = "UPDATE co_rr SET 
        ...
        if (mysqli_query($con, $update_query)) {
            ...
            echo "<script>alert('Error updating result page: " . mysqli_error($con) . "');</script>";
        }
    } else {
        // Insert new result page - status defaults to 'Active' from table definition
        $insert_query = "INSERT INTO co_rr (hero_title, hero_subtitle) 
...
if (isset($_GET['toggle_id'])) {
    ...
    $toggle_query = "UPDATE co_rr SET status = IF(status = 'Active', 'Inactive', 'Active') WHERE id = '$toggle_id'";
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM co_rr WHERE id = '$delete_id'";
    if (mysqli_query($con, $delete_query)) {
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
        echo "<script>alert('Error deleting result page: " . mysqli_error($con) . "');</script>";
    }
}

// Fetch all result pages from database for display
$rrData = [];
$select_query = "SELECT * FROM co_rr ORDER BY id ASC";
