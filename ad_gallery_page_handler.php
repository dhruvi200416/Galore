...
// Create gallery_page table
$gallery_table = "CREATE TABLE IF NOT EXISTS gallery_page (
...
if (mysqli_query($con, $gallery_table)) {
    // echo "Table 'gallery_page' created successfully";
} else {
    // echo "Table not created: " . mysqli_error($con);
}

// Handle form submission for gallery page
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE gallery_page SET 
        ...
        if (mysqli_query($con, $update_query)) {
            ...
            echo "<script>alert('Error updating gallery: " . mysqli_error($con) . "');</script>";
        }
    } else {
        // Insert new gallery
        $insert_query = "INSERT INTO gallery_page (hero_title, hero_subtitle) 
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM gallery_page WHERE id = '$delete_id'";
    if (mysqli_query($con, $delete_query)) {
        header("Location: " . $_SERVER['PHP_SELF']);
        exit();
    } else {
        echo "<script>alert('Error deleting gallery: " . mysqli_error($con) . "');</script>";
    }
}

// Fetch all galleries from database for display
$galleryData = [];
$select_query = "SELECT * FROM gallery_page ORDER BY id ASC";
