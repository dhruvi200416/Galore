...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3" id="topBar">
            <h1 class="mb-0"><i class="fas fa-tachometer-alt me-2"></i>Coordinator Dashboard Management</h1>
            <!-- <button class="btn-add-dash" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add Dashboard Page
            </button> -->
        </div>

        <!-- ADD / EDIT DASHBOARD FORM -->
        <div class="add-dash-form-container" id="addDashForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add Dashboard Page</h3>
        ...
        <div class="data-table-container">
            <div class="table-header" id="tableHeader">
                <h2 class="mb-0">Dashboard Pages List</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold"><i class="fas fa-info-circle me-2"></i>Dashboard Page Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="text-center mb-4">
                        <i class="fas fa-tachometer-alt" style="font-size: 4rem; color: var(--galore-red); opacity: 0.5;"></i>
                    </div>
                    <div class="row g-3">
                        <div class="col-12">
    ...
    <script>
        // Use PHP to pass database dashboards to JavaScript
        let dashData = <?php echo json_encode($dashData); ?>;

        // If no dashboards in database, use empty array
        ...
        const addDashForm = $("#addDashForm");
        const formTitleText = $("#formTitleText");

        // Open form button
        $("#openFormBtn").click(() => {
            formTitleText.text("Add Dashboard Page");
            editId = null;
            $("#edit_id").val("");
            $("#dashForm")[0].reset();
            ...
            addDashForm.slideDown(300);
        });

        // Cancel form button
        $("#cancelForm").click(() => {
            addDashForm.slideUp(300);
            $("#dashForm")[0].reset();
            $("#edit_id").val("");
            // Clear validation classes and error messages
            $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
        ...
        function renderTable() {
            let tbody = $("#dashTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = dashData.filter(d =>
                d.hero_title.toLowerCase().includes(searchValue) ||
                d.hero_subtitle.toLowerCase().includes(searchValue)
            );
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(d => {
                    const statusClass = d.status === 'Active' ? 'btn-status-active' : 'btn-status-inactive';
                    
                    tbody.append(`
                    <tr>
                        <td><span ">${d.id}</span></td>
                        <td>${d.hero_title}</td>
                        <td>${d.hero_subtitle}</td>
                        <td>
                            <button class="btn-status ${statusClass}" onclick="toggleStatus(${d.id})">
                                ${d.status}
                            </button>
                        </td>
                        <td>
                            <button class="action-btn btn-view" onclick="viewDash(${d.id})"> View</button>
                            <button class="action-btn btn-edit" onclick="editDash(${d.id})"> Edit</button>
                            <button class="action-btn btn-delete" onclick="deleteDash(${d.id})"> Delete</button>
                        </td>
                    </tr>`);
                });
            }
        ...
        window.viewDash = function(id) {
            let d = dashData.find(x => x.id == id);
            $("#v_id").text(d.id);
            $("#v_hero_title").text(d.hero_title);
            $("#v_hero_subtitle").text(d.hero_subtitle);
            $("#v_status").text(d.status);
            new bootstrap.Modal(document.getElementById("viewModal")).show();
        };

        // Delete dashboard
        window.deleteDash = function(id) {
            if (confirm("Are you sure you want to delete this dashboard page?")) {
        ...
        window.editDash = function(id) {
            editId = id;
            const d = dashData.find(x => x.id == id);
            formTitleText.text("Edit Dashboard Page");
            $("#edit_id").val(id);
            $('input[name="hero_title"]').val(d.hero_title);
            $('input[name="hero_subtitle"]').val(d.hero_subtitle);
            // Status field removed from form - don't set it
        ...
        $(document).ready(function() {
            ...
            // Dashboard form submission with validation
        ...
        $(document).keydown(function(e) {
            if (e.key === "Escape" && addDashForm.is(":visible")) {
                addDashForm.slideUp(300);
                $("#dashForm")[0].reset();
                $("#edit_id").val("");
                $(".is-valid, .is-invalid").removeClass("is-valid is-invalid");
