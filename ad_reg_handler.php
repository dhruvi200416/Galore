...
if (mysqli_query($con, $create_db)) {
    // Database created/checked successfully
} else {
...
// Create reg_header table
$reg_header_table = "CREATE TABLE IF NOT EXISTS reg_header (
...
if (!mysqli_query($con, $reg_header_table)) {
    // Log error but continue
    error_log("Table creation error: " . mysqli_error($con));
}

// Handle form submission for registration header
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['edit_id']) && !empty($_POST['edit_id'])) {
        ...
        $update_query = "UPDATE reg_header SET 
        ...
        // Insert new header
        $insert_query = "INSERT INTO reg_header (hero_title, hero_subtitle, form_title, event_date, footer_note) 
...
if (isset($_GET['delete_id'])) {
    ...
    $delete_query = "DELETE FROM reg_header WHERE id = '$delete_id'";
...
$headers = [];
$select_query = "SELECT * FROM reg_header ORDER BY id ASC";
