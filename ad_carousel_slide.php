...
include 'ad_carousel_handler.php';
?>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="top-bar">
            <h1 class="mb-0">Carousel Slides Management</h1>
            <button class="btn-add-slide" id="openFormBtn">
                <i class="bi bi-plus-circle"></i> Add New Slide
            </button>
        </div>

        <!-- ADD SLIDE FORM -->
        <div class="add-slide-form-container" id="addSlideForm" style="display:none;">
            <h3 class="form-title" id="formTitleText">Add New Slide</h3>

            <form id="slideForm" method="POST" enctype="multipart/form-data" action="ad_carousel_handler.php">
                ...
                <div class="galore-input-group">
                    ...
                    <input type="text" name="title" class="galore-input" data-validation="required min max" data-min="5" data-max="100">
                    <span id="title_error" class="error-message"></span>
                </div>

                <div class="form-buttons">
                    <button type="submit" name="submit" class="btn-save">Save Slide</button>
        ...
        <div class="data-table-container">
            <div class="table-header">
                <h2 class="mb-0">Carousel Slides Data</h2>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:18px;border-top:6px solid var(--galore-red);">
                <div class="modal-header text-white" style="background:var(--galore-red);">
                    <h5 class="modal-title fw-bold">Slide Full Details</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>

                <div class="modal-body text-center">
                    <div id="v_image_container" class="modal-image-container" style="display:none;">
                        <img id="v_image" class="modal-image" src="#" alt="Slide">
                    </div>
    ...
    <script>
        // Use PHP to pass database slides to JavaScript
        let slidesData = <?php echo json_encode($slides); ?>;

        // If no slides in database, use empty array
        ...
        const addSlideForm = $("#addSlideForm");
        ...
        $("#openFormBtn").click(() => {
            formTitleText.text("Add New Slide");
            editId = null;
            isEditing = false;
            $("#edit_id").val("");
            $("#slideForm")[0].reset();
            ...
            $(".error-message").text("");
            addSlideForm.slideDown();
        });

        $("#cancelForm").click(() => {
            addSlideForm.slideUp();
            $("#slideForm")[0].reset();
            $("#edit_id").val("");
            $("#file-name").text("Choose file...");
            imagePreviewContainer.style.display = "none";
        ...
        function renderTable() {
            let tbody = $("#slideTableBody");
            tbody.html("");

            const searchValue = $("#searchInput").val().toLowerCase();
            const filteredData = slidesData.filter(s =>
                String(s.title).toLowerCase().includes(searchValue) ||
                String(s.alt_text || "").toLowerCase().includes(searchValue)
            );
            ...
            if (paginatedData.length === 0) {
                ...
                paginatedData.forEach(s => {
                    const imageDisplay = s.image && s.image.trim() !== "" ? 
                        `<div class="table-image-container">
                            <img src="${s.image}" class="table-image" alt="${s.title}">
                        </div>` : 
                        "<div class='table-image-container' style='background:var(--galore-gray);color:white;font-size:20px;'><i class='bi bi-image'></i></div>";
                    ...
                    tbody.append(`
                        <tr>
                            ...
                            <td>${s.title}</td>
                            <td>
                                <button class="btn-status ${s.status === 'Active' ? 'btn-status-active' : 'btn-status-inactive'}" 
                                        onclick="toggleStatus(${s.id})">
                                    ${s.status}
                                </button>
                            </td>
                            <td>
                                <button class="action-btn btn-view" onclick="viewSlide(${s.id})">View</button>
                                <button class="action-btn btn-edit" onclick="editSlide(${s.id})">Edit</button>
                                <button class="action-btn btn-delete" onclick="deleteSlide(${s.id})">Delete</button>
                            </td>
                        </tr>
                    `);
                });
        ...
        function toggleStatus(id) {
            window.location.href = "ad_carousel_handler.php?toggle_id=" + id;
        }
        ...
        function editSlide(id) {
            ...
            formTitleText.text("Edit Slide");
            $("#edit_id").val(id);

            // Fill form with slide data
            $('input[name="title"]').val(slide.title);
            ...
            if (slide.image) {
                ...
                imagePreview.src = currentImage;
                imagePreviewContainer.style.display = "flex";
                $("#file-name").text("Current image: " + slide.image.split('/').pop());
            }
        ...
        function deleteSlide(id) {
            if (confirm("Are you sure you want to delete this slide?")) {
                window.location.href = "ad_carousel_handler.php?delete_id=" + id;
            }
        }
        ...
        function viewSlide(id) {
            ...
            if (slide) {
                ...
                if (slide.image) {
                    vImage.attr("src", slide.image);
                    vImageContainer.show();
                } else {
                    vImageContainer.hide();
                }
                
                $("#v_title").text(slide.title);
                $("#v_status").text(slide.status);
        ...
        $(document).keydown(function(e) {
            if (e.key === "Escape" && addSlideForm.is(":visible")) {
                addSlideForm.slideUp();
                $("#slideForm")[0].reset();
                $("#file-name").text("Choose file...");
                imagePreviewContainer.style.display = "none";
